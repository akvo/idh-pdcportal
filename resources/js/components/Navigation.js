    ...
    return filters.map((c) => {
      ...
      return (
        <NavDropdown.Item key={c.name} as={Link} to={url}>
          {c?.childrens?.[0]?.country_name || c.name}
        </NavDropdown.Item>
      );
    });
  }
              ...
              {user.login ? (
                ...
                >
                  <NavDropdown.Item as={Link} to="/setting">
                    <FontAwesomeIcon className="mr-2" icon={["fas", "cogs"]} />{" "}
                    Settings
                  </NavDropdown.Item>
                  {user.role === "admin" ? (
                    <>
                ...
                <Nav.Link as={Link} to="/login">
                  <FontAwesomeIcon className="mr-2" icon={["fas", "lock"]} />{" "}
                  {loginText}
                </Nav.Link>
              )}
            </Nav.Item>
